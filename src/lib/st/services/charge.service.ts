...
export async function chargeForBuild(
  ...
  try {
    ...
    if (!updateError) {
      ...
      // ✅ Process 'build' referral reward for referrer ($1.00 reward)
      try {
        ...
        if (referral) {
          ...
          if (!refUpdateErr) {
            ...
            await supabase.from("ledger_entries").insert({
              ...
              amount: rewardAmount,
              type: "referral_bonus",
              description: `Build referral reward for site build by referee ${userId}`,
              reference_id: referral.id,
            });
          }
        }
      } catch (refErr) {
        console.error("Build referral processing error:", refErr);
      }
    }
