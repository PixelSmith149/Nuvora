...
import { TwoFactorChallengeModal } from "./TwoFactorChallengeModal"; // adjust path
...
// Clear stale app-lock cookie from any previous session
try {
  await fetch("/api/auth/app-lock", {
    method: "DELETE",
  });
} catch {
  // Ignore
    ...
    try {
      ...
      const twoFactorEnabled =
        settingsData?.settings?.twoFactorEnabled === true;

      if (twoFactorEnabled) {
        // Show challenge modal — do NOT redirect yet
      ...
      // Fallback: if settings check fails, still allow login
      // (or you can choose to block — up to you)
  ...
  // Called when the user successfully enters the TOTP code
  ...
  // Called when user cancels the challenge
  async function handle2FACancel() {
    // Sign the user out so they don't stay half-authenticated
