...
import { verifyAdminPasskey } from "./action"; // 🎯 Ingest the server action code
...
export default function AdminLoginPage({ params }: AdminLoginPageProps) {
	...
	const handleVerificationSubmit = async (e: React.FormEvent) => {
		...
		console.log("🔑 Expected passkey:", SECRET_ADMIN_TOKEN);
		console.log("🔑 Match:", passkey === SECRET_ADMIN_TOKEN);

		try {
			// 🎯 Dispatch input value directly to the secure server verification pipe
			const isAuthorized = await verifyAdminPasskey(passkey);

			if (isAuthorized) {
				// 🏁 Give the browser a brief 150ms window to commit the cookie to storage
				await new Promise((resolve) => setTimeout(resolve, 150));

				// Now force the top-level window redirect with the saved cookie state
				window.location.href = `/${tech}/admin-dashboard`;
			} else {
				setError(true);
