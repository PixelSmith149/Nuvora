// app/m/[username]/onboarding/page.tsx
...
export default function OnboardingPage({ params }: OnboardingPageProps) {
	...
	useEffect(() => {
		async function checkAuth() {
			try {
				...
				if (!currentUser) {
					router.push(`/auth/login?redirect=/m/${username}/onboarding`);
					return;
				}
				...
				const { data: storeData, error: storeError } = await supabase
					...
					.select("is_verified, terms_accepted_at")
					.eq("user_id", currentUser.id)
					.single();

				// ✅ If user already has a verified store, redirect to their store
				if (storeData?.is_verified === true) {
					router.push(`/m/${username}/store`);
					return;
				}

				// ✅ If user already accepted terms but not verified yet, redirect to store
				if (storeData?.terms_accepted_at && storeData?.is_verified === false) {
					router.push(`/m/${username}/store`);
					return;
				}
			} catch (error) {
	...
	const handleComplete = async () => {
		...
		try {
			...
			const { error } = await supabase.from("global_market_stores").upsert(
				{
					user_id: user?.id,
	...
	return (
		<div className="min-h-screen bg-black text-white">
			<div className="mx-auto w-full max-w-4xl px-4 py-6 sm:px-6 lg:px-8">
				<div className="rounded-2xl border border-white/5 bg-zinc-950/40 shadow-2xl">
								...
								<div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
									...
									>
										<Button
											...
											onClick={
												currentStep === "welcome"
													? () => router.push(`/m/${username}/account`)
													: goToPrev
											}
