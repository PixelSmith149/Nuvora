...
export default function PaystackPage() {
  ...
  // Any supported display currency (EUR, GBP, NGN, USD, GHS, …)
  ...
  // Preview: selected currency → USD ledger + GHS charge (client estimate only)
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const from = selectedCurrency.toUpperCase() as Currency;

        // Any currency → USD (wallet)
        const usd = await convertToUSD(value, from);

        // USD → GHS (what Paystack will charge) — or same if already GHS
        const ghs =
          from === "GHS"
            ? value
            : (await convertFromUSD(usd, "GHS")).amount;

        if (!cancelled) {
          setPreviewUsd(Number.isFinite(usd) ? usd : null);
          setPreviewGhs(Number.isFinite(ghs) ? ghs : null);
        }
      } catch (err) {
        console.error("Preview FX failed:", err);
        if (!cancelled) {
          setPreviewGhs(null);
          setPreviewUsd(null);
        }
  ...
  const previewText = useMemo(() => {
    if (previewLoading) return "loading" as const;
    if (previewGhs == null || previewUsd == null) return null;
    return {
      ghs: previewGhs.toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }),
      usd: previewUsd.toLocaleString(undefined, {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }),
    };
  }, [previewGhs, previewUsd, previewLoading]);
  ...
  async function handlePayment() {
    ...
    try {
      // Send display currency + amount only.
      // Server converts to GHS for Paystack and USD for the ledger.
      ...
      <section className="flex-1 flex items-center justify-center px-4 sm:px-6">
        <div className="w-full max-w-md border border-white/10 rounded-3xl p-6 sm:p-8 text-center bg-zinc-950/60 backdrop-blur-xl mt-20 relative">
          ...
          <p className="text-zinc-500 mt-1.5 text-xs sm:text-sm">
            Enter an amount in any currency. You pay in{" "}
            <span className="text-white font-semibold">GHS</span> via Paystack.
            Your wallet is credited in{" "}
          ...
          {status === "success" ? (
            ...
            <>
              {/* All currencies from GLOBAL_SUPPORTED_CURRENCIES */}
              ...
              <div className="relative mt-6 w-full">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 font-black text-base font-mono select-none">
                  {selectedCurrency}
                </span>
                <input
                  type="number"
                  ...
                  style={{
                    paddingLeft: `${Math.max(
                      selectedCurrency.length * 12 + 24,
                      72,
                    )}px`,
                  }}
                  className="w-full pr-4 py-4 rounded-xl bg-black border border-white/10 text-left text-lg outline-none font-mono text-white focus:border-white/30 transition-all"
                />
              </div>

              {/* Conversion preview */}
              <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-left text-xs space-y-1.5 min-h-[4.5rem]">
                {previewText === "loading" && (
                  <p className="text-zinc-500">Calculating conversion…</p>
                )}

                {previewText && previewText !== "loading" && (
                  <>
                    <p className="text-zinc-500">
                      You enter{" "}
                      <span className="text-white font-mono font-bold">
                        {selectedCurrency} {Number(amount).toLocaleString()}
                      </span>
                    </p>
                    <p className="text-zinc-500">
                      You pay (Paystack){" "}
                    ...
                    <p className="text-zinc-600 text-[10px] pt-1">
                      Final rates are set on the server at checkout. Foreign
                      cards may show a local amount from your bank.
                    </p>
                  </>
                )}

                {!previewText && !previewLoading && Number(amount) > 0 && (
                  <p className="text-zinc-500">Unable to load rates right now.</p>
                )}
              </div>
          ...
          <p className="text-zinc-600 text-[10px] mt-4 font-mono">
            Any currency in · Charged in GHS · Credited in USD
          </p>
        </div>
      </section>
    </main>
